...
### Requirement: K3s cluster provisioning script (legacy)
**Reason**: Replaced by `deploy/k3s/k3s-provision.sh` (K3s + cert-manager only, infrastructure is in `k3s-infrastructure.sh`).
**Migration**: Use `deploy/k3s/k3s-provision.sh` + `deploy/k3s/k3s-infrastructure.sh` instead of `scripts/provision-k3s.sh`.

### Requirement: Infrastructure provisioning script (legacy)
**Reason**: Replaced by `deploy/k3s/k3s-infrastructure.sh` which uses the same Helm charts but with secrets generated via `openssl rand -hex 32` and idempotent VPS execution.
**Migration**: Use `deploy/k3s/k3s-infrastructure.sh`. The old `scripts/provision-infra.sh` is kept for reference only.
