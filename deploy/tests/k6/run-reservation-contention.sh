...
echo "[*] Creando evento con zonas (zone-general: 5000, zone-vip: $ZONE_CAPACITY)..."
EVENT_ID=$(curl -s "$GRAPHQL_URL" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -d "{\"query\":\"mutation { createEvent(input: { name: \\\"Concierto Test K6\\\", type: CONCERT, date: \\\"$FUTURE_DATE\\\", venueId: \\\"$VENUE_ID\\\", artistIds: [\\\"$ARTIST_ID\\\"], zones: [{ id: \\\"zone-general\\\", name: \\\"General\\\", capacity: 5000, price: 50.0 }, { id: \\\"$ZONE_ID\\\", name: \\\"VIP\\\", capacity: $ZONE_CAPACITY, price: 150.0 }] }) { id zones { id name capacity } } }\"}" \
...
echo ""
echo "[*] Ejecutando k6 reservation-contention ($K6_VUS VUs, $K6_ITERATIONS iteraciones, zona=$ZONE_ID con capacidad=$ZONE_CAPACITY, $K6_USERS usuarios)..."
k6 run \
  --vus $K6_VUS \
